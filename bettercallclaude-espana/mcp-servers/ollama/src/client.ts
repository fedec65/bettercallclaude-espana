  ...
  getRecommendations(availableModels: OllamaModel[]): ModelRecommendation[] {
    ...
    if (legal) {
      recommendations.push({
        model: legal,
        task: 'Spanish legal analysis',
        reason: 'Large model suitable for complex legal reasoning and bilingual (ES/EN) content',
      });
    }
    ...
    if (multilingual) {
      recommendations.push({
        model: multilingual,
        task: 'Multilingual legal text (ES/EN)',
        reason: 'Strong multilingual capabilities for bilingual Spanish legal documents',
      });
    }
