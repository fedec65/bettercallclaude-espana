...
/**
 * Ollama MCP Server
 * Local LLM Processing for Spain Legal Privacy Protection
 *
 * Enables local processing of privileged legal content via Ollama,
 * enforcing Art. 24 LOPJ / Art. 542 CP (secreto profesional) compliance.
 *
 * Tools:
 ...
 * - ollama_classify_privacy: Classify text by Spanish privacy level (offline)
...
- Model recommendations for Spanish legal work
...
Use this before attempting generate or chat to verify Ollama is available.`,
      ...
      },
      {
        ...
        description: `Generate text locally using Ollama for privacy-sensitive Spanish legal content.

Use this for PRIVILEGED content (Art. 24 LOPJ / Art. 542 CP) that must not be sent to cloud APIs:
...
Supports optional system prompt for context and generation options.`,
        ...
        inputSchema: {
          ...
          properties: {
            ...
            system_prompt: {
              type: 'string',
              description: 'Optional system prompt for context (e.g., "You are a Spanish legal assistant")',
            },
            temperature: {
              type: 'number',
...
Requires Ollama to be running locally (ollama serve).`,
      ...
      },
      {
        ...
        description: `Classify text by Spanish legal privacy level using pattern detection.

Works entirely OFFLINE — no Ollama or network connection required.

Detects patterns in Spanish and English:
- PRIVILEGED: secreto profesional, secreto de las comunicaciones, secreto del abogado,
  privilegio de la defensa, Art. 24 LOPJ, Art. 542 CP, attorney-client privilege, etc.
- CONFIDENTIAL: confidencial, reservado, privado, confidential, etc.
...
Returns the classification level, matched patterns, and routing guidance.`,
      ...
      },
      {
        ...
        description: `List installed Ollama models with Spanish legal task recommendations.

Returns:
- All installed models with name, size, and quantization details
- Task-specific recommendations for Spanish legal work:
  - Legal analysis (large models: mixtral, llama3)
  - Quick classification (small models: phi, gemma)
  - Multilingual ES/EN (aya, mixtral, qwen)
              ...
              text: JSON.stringify(
                {
                  ...
                  routing: {
                    ...
                    PRIVILEGED: 'Local processing ONLY (Art. 24 LOPJ / Art. 542 CP)',
                  }[result.level],
                },
                null,
                2
